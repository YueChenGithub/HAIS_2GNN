...
def get_model(args):
    # initiate model, replace int(args.use_height + 3) with int args.use_height to fix 
    # feature size and kernel size not match 
    input_channels = int(args.use_multiview) * 128 + int(args.use_normal) * 3 +\
                     int(args.use_color) * 3 + int(args.use_height)
    ...
    if args.use_pretrained:
        ...
        pretrained_path = os.path.join(args.use_checkpoint, "model.pth")
...
def get_solver(args, dataloader):
    ...
    if args.use_checkpoint:
        ...
        checkpoint = torch.load(os.path.join(args.use_checkpoint, "checkpoint.tar"))
