...
class RelationModule(nn.Module):
    def __init__(self, input_feature_dim, args, v_dim=256, h_dim=128, l_dim=256, dropout_rate=0.15):
        super().__init__()
        ...
        self.gcn2 = DynamicEdgeConv2(3 + input_feature_dim + args.num_classes, 128, k=args.k,
                                   num_classes=args.num_classes)
    ...
    def forward(self, data_dict):
        ...
        feats1 = self.gcn(support_xyz, batch_index, filtered_index, feats)

        feats2 = self.gcn2(support_xyz, batch_index, filtered_index, feats, batch_size=len(lang_feats))

        feats = torch.concat((feats1, feats2), dim=1)
