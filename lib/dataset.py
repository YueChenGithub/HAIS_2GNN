...
class ScannetReferenceDataset(Dataset):
    ...
    def __init__(self, scanrefer, scanrefer_all_scene, split="train", args=None):
        ...
        self.voxel_size_ap = tuple(np.array([self.args.voxel_size_ap, self.args.voxel_size_ap, self.args.voxel_size_ap]))
        self.voxel_size_glp = tuple(np.array([self.args.voxel_size_glp, self.args.voxel_size_glp, self.args.voxel_size_glp]))
    ...
    def __getitem__(self, idx):
        ...
        for i_instance in np.unique(instance_labels):
            ...
            if ins_class in DC.nyu40ids:
                ...
                ##  x 1024*7 careful with the feature length
                    pc = x[:, :3]
                    coords, indices = sparse_quantize(pc,voxel_size=self.voxel_size_ap,return_index=True)
                    feats = torch.tensor(x[:,3:][indices],dtype=torch.float)
        ...
        pc = point_cloud[:, :3]
        coords, indices = sparse_quantize(pc,voxel_size=self.voxel_size_glp,return_index=True)
        ##
