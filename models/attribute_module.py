...
# replace GlobalMaxPooling with GlobalMaxPool
class AttributeModule(nn.Module):
        ...
        for i in range(batch_size):
            ...
            for j in range(num_obj):
                if instance_class[j] == lang_cls_pred[i]:
                    ...
                    coords, indices = sparse_quantize(pc,voxel_size=self.voxel_size,return_index=True)
    ...
    def forward(self, data_dict):
        ...
        if pts_batch==[]:
          return 0
        feats = sparse_collate(pts_batch).cuda()
